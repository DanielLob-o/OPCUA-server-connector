...
class Client(object):
    ...
    def connect(self):
        ...
        try:
            ...
            try:
                self.create_session()
                try:
                    self.activate_session(username=self._username, password=self._password,
    ...
    def open_secure_channel(self, renew=False):
        ...
        params.ClientNonce = utils.create_nonce(
            self.security_policy.symmetric_key_size)  # this nonce is used to create a symmetric key
    ...
    def _add_certificate_auth(self, params, certificate, challenge):
        ...
        if certificate.signature_hash_algorithm.name == "sha256":
            params.UserIdentityToken.PolicyId = self.server_policy_id(ua.UserTokenType.Certificate,
    ...
    def get_values(self, nodes):
        ...
        nodes = [ua.NodeId.from_string(node) for node in nodes]
    ...
    def set_values(self, nodes, values):
        """
        Write values of type boolean to multiple nodes in one ua call
        """
        nodeids = [ua.NodeId.from_string(node) for node in nodes]
        dvs = [ua.DataValue(ua.Variant(val, ua.VariantType.Boolean)) for val in values]
        results = self.uaclient.set_attributes(nodeids, dvs, ua.AttributeIds.Value)
        for result in results:
            result.check()

    def set_values2(self, nodes, values):
        """
        Write values of type int16 to multiple nodes in one ua call
        """
        nodeids = [ua.NodeId.from_string(node) for node in nodes]
        dvs = [ua.DataValue(ua.Variant(val, ua.VariantType.UInt16)) for val in values]
        results = self.uaclient.set_attributes(nodeids, dvs, ua.AttributeIds.Value)
        for result in results:
            result.check()

    def set_values3(self, nodes, values):
        """
        Write values of type int16 to multiple nodes in one ua call
        """
        nodeids = [ua.NodeId.from_string(node) for node in nodes]
        dvs = [ua.DataValue(ua.Variant(val, ua.VariantType.Int16)) for val in values]
        results = self.uaclient.set_attributes(nodeids, dvs, ua.AttributeIds.Value)
        for result in results:
            result.check()

    def set_values4(self, nodes, values):
        """
        Write values of type int16 to multiple nodes in one ua call
        """
        nodeids = [ua.NodeId.from_string(node) for node in nodes]
        dvs = [ua.DataValue(ua.Variant(val, ua.VariantType.Float)) for val in values]
        results = self.uaclient.set_attributes(nodeids, dvs, ua.AttributeIds.Value)
        for result in results:
            result.check()
